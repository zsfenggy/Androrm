...
public abstract class ModelCache {
	...
	public static <T extends Model> boolean knowsFields(Class<T> clazz) {
		 return  knowsModel(clazz) &&
			!KNOWN_MODEL_FIELDS.get(DatabaseBuilder.getTableName(clazz)).isEmpty();
	}
	...
	public static <T extends Model> boolean modelHasField(Class<T> clazz, String field) {
		return knowsFields(clazz) &&
			 KNOWN_MODEL_FIELDS.get(DatabaseBuilder.getTableName(clazz)).contains(field);
	}
